...
virtualPortRouterInprocAddress = "virtualPortRouter" + virtualPortRouterExtension;
...
//Now bind the socket
SOM_TRY
virtualPortServer->bind(("inproc://" + parentVirtualPortRouter->virtualPortRouterInprocAddress).c_str());
...
std::unique_ptr<zmq_pollitem_t[]> pollItems(new zmq_pollitem_t[numberOfPollObjects]);
...
//UDP
pollItems[0] = {nullptr, parentVirtualPortRouter->datagramPortNumber, ZMQ_POLLIN, 0 };
pollItemLocationToVirtualPortNumber[0] = -1;

//Virtual port creation/destruction socket
pollItems[1] = {virtualPortServer.get(), 0, ZMQ_POLLIN, 0 };
...
{
pollItems[index] = {iter->second, 0, ZMQ_POLLIN, 0 };
pollItemLocationToVirtualPortNumber[index] = iter->first;
index++;
}

//Listen for activity for 100 milliseconds
SOM_TRY
...
{
//Call process remove or add pair function
}
...
@execptions: This function can throw exceptions
*/
void virtualPortRouterResources::processUDPMessage()
{
...
//Check to make sure the result is not too large
...
*(pointerToMessageHeader+inputVirtualPortMessageHeader.ByteSize()) = htonl(inputVirtualPortMessageHeader.ByteSize());
