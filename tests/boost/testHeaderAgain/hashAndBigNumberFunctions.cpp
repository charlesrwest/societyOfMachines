...
{
return inputBigInteger.str(-1,std::ios::hex);
}
...
{
return inputBigInteger.str(-1,std::ios::dec); 
}
...
for(int i=0; i<(128-hexVersionOfBigInteger.size()); i++)
{
buffer.push_back('0');
}
buffer+=hexVersionOfBigInteger;

return convertHexStringToBinary(buffer.c_str(), buffer.size(), inputRawHashBuffer, inputRawHashBufferSize);
}
