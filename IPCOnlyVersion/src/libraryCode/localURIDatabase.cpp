...
std::string removeURIWithResourceNameCommand = "delete from URIs where resourceName = ?100;";
if(sqlite3_prepare_v2(databaseConnection, removeURIWithResourceNameCommand.c_str(), removeURIWithResourceNameCommand.size(), &removeURIWithResourceNameStatement, NULL) != SQLITE_OK)
{
...
{
fprintf(stderr, "Error preparing parametric sql statement9: %s\n", sqlite3_errstr(sqlite3_extended_errcode(databaseConnection)));
isValid = false;
return;
}
...
@return: 1 if successful and 0 otherwise
*/
int localURIDatabase::removeURI(const std::string &inputResourceName)
{
//This function/statement removes the URI and its associated tags by taking advantage of foreign key constraints and cascade deletes.  It won't work if they haven't been enabled by a pragma command (but that should have happened in the constructor) 


if(sqlite3_bind_text(removeURIWithResourceNameStatement, 100, inputResourceName.c_str(), inputResourceName.size(), SQLITE_TRANSIENT) != SQLITE_OK)
{
fprintf(stderr, "Error, unable to bind prepared statement for removing URI with resource name (%s)\n", inputResourceName.c_str());
reportLastDatabaseError();
return 0;
}

//We successfully bound the value to the query, so now we execute it
if(sqlite3_step(removeURIWithResourceNameStatement) != SQLITE_DONE)
{
fprintf(stderr, "Error, unable to step delete URI\n");
reportLastDatabaseError();
return 0;
...
//The step completed successfully, so now we reset the prepared statement for later use
if(sqlite3_reset(removeURIWithResourceNameStatement) != SQLITE_OK)
{
fprintf(stderr, "Error, unable to reset statement\n");
reportLastDatabaseError();
return 0;
